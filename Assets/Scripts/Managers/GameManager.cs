...
namespace Managers
{
    public class GameManager : MonoBehaviour
    {
        ...
        {
            get => _currentQuota;
            private set => _currentQuota = value;
        }
        ...
        {
            // THE QUOTA CHECK
            if (Player.Inventory.money < CurrentQuota)
            {
            ...
            if (CurrentDay == 15)
            {
                UIManager.Instance.TriggerDay15Win();
                return;
            }
        ...
        private void CalculateNextQuota()
        {
            ...
            var randomVariance =
                UnityEngine.Random.Range(0.8f,
                    1.4f); // Add some randomness to the quota so it's not the same every time
        ...
        private void TogglePause(InputAction.CallbackContext ctx)
        {
            if (SceneManager.GetActiveScene().name != "Mines")
                return; // Don't allow pausing in the lobby or main menu, why the hell would you do that, idiot, stupid
